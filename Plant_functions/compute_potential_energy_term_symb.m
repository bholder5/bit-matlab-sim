function Pot = compute_potential_energy_term_symb(theta, c_n, z_n, m_n, r_n1_n, g0)
    ...
    %rotation matricies
    C_n = sym(zeros(3,3,ndof));
    C_n_rate = C_n;
    % Potential energy
    Pot = sym(zeros(ndof,1));
    
    for n = 1:ndof
        C_n(:,:,n) = axis2rot_symb(z_n(:,n), theta(n));
